...
public class ItemHoverInfoDisplayManager : MonoBehaviour
{
    ...
    private void DisplayInfo(GameEventType gameEventType, object[] data)
    {
        ...
        bool isNoItemButtonOrNoAssosiatedItem = _currentDisplayedItemButton == null || _currentDisplayedItemButton.AssosiatedItem == null;

        if (isNoItemButtonOrNoAssosiatedItem)
        {
    ...
    {
        _itemId = $"{GlobalFunctions.TextWithColorCode(_whiteText, "ID:")} {GlobalFunctions.TextWithColorCode(_partiallyTransparentText, _currentDisplayedItemButton.AssosiatedItem.ID.ToString())}";
        _itemOriginalPrice = $"{GlobalFunctions.TextWithColorCode(_whiteText, "Original Price:")} {GlobalFunctions.TextWithColorCode(_partiallyTransparentText, "$" + _currentDisplayedItemButton.AssosiatedItem.Price.ToString())}";
